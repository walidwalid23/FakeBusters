...
class _SearchState extends ConsumerState<Search> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:  TextField(
          ...
          onSubmitted: (String productName){
            ref.read(searchPostsByProductNameProvider(context).notifier).searchPostsByProductNameState(productName);
      ...
      body: ref.watch(searchPostsByProductNameProvider(context)).when(
          ...
          loading: ()=> Align(
          alignment: Alignment.center,
          child: RotationTransition(turns: _animationController, child: Image.asset('assets/images/search.png'))),
    ));
  }
